...
function preload(){
  ...
  balloonSound = loadSound("Balloon popping _ burst.mp3")
...
if (arrowGroup.isTouching(redballoonGroup)){  arrowGroup.destroyEach();
   redballoonGroup.destroyEach();
                                            balloonSound.play( );
  score=score+10
   }
  if (arrowGroup.isTouching(blueballoonGroup)){  arrowGroup.destroyEach();
                                               balloonSound.play( );
                                        blueballoonGroup.destroyEach();
  score=score+20
   }
  if (arrowGroup.isTouching(pinkballoonGroup)){  arrowGroup.destroyEach();
pinkballoonGroup.destroyEach();
  balloonSound.play( );                      
                                               score=score+8
   }
    if(arrowGroup.isTouching(greenballoonGroup)){  arrowGroup.destroyEach();
